...
try:
    ...
    with c2:
        st.metric("Optimal Clusters (k)", f"{df_seg['cluster_id'].nunique()}")
    with c3:
        st.metric("Top Segment Share", f"{(df_seg['cluster_id'] == 0).mean()*100:.1f}%")
    with c4:
        st.metric("Repeat Segment Share", f"{(df_seg['cluster_id'] == 1).mean()*100:.1f}%")
    ...
    with col_a:
        ...
        seg_counts = df_seg["cluster_description"].value_counts().reset_index()
    ...
    with col_b:
        ...
        rev_seg = df_seg.groupby("cluster_description")["total_revenue"].sum().reset_index()
        fig_rev_seg = px.bar(
            rev_seg,
    ...
    sample_id = df_seg["customer_id"].iloc[0]
    ...
    if st.button("Predict Customer Segment"):
        ...
        match = df_seg[(df_seg["customer_id"] == cust_input) | (df_seg["customer_unique_id"] == cust_input)]
        if not match.empty:
