...
try:
    ...
    with c4:
        st.metric("Repeat Buyers Rate", f"{(df_rp['repeat_customer'] == 1).mean()*100:.2f}%")
    ...
    sample_id = df_rp["customer_id"].iloc[0]
    ...
    if st.button("Predict Repeat Propensity"):
        ...
        match = df_rp[(df_rp["customer_id"] == cust_input) | (df_rp["customer_unique_id"] == cust_input)]
        if not match.empty:
